...
$disableMethod = "RelocateVM_Task"

$viObject | ForEach-Object {
  $vmMoRef = $_.ExtensionData.MoRef.Value

  # vSphere MOB URL to private disableMethods
  $mob_url = "https://$viServer/mob/?moid=AuthorizationManager&method=disableMethods"
...
"@
  ...
  $body = @"
vmware-session-nonce=$sessionnonce&entity=%3Centity+type%3D%22ManagedEntity%22+xsi%3Atype%3D%22ManagedObjectReference%22%3E$vmMoRef%3C%2Fentity%3E%0D%0A%0D%0A&method=%3CDisabledMethodRequest%3E%0D%0A+++%3Cmethod%3E$disableMethod%3C%2Fmethod%3E%0D%0A%3C%2FDisabledMethodRequest%3E%0D%0A%0D%0A&sourceId=self
"@

  Write-Verbose "$($_.Name) - Disabling Method $disableMethod" -Verbose
